<nav x-data="{ open: false }"
    class="bg-white/70 dark:bg-gray-800/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-700 sticky top-0 z-50">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @auth
                        ...
                        @elseif(Auth::user()->isRecruiter())
                            ...
                            <x-nav-link :href="route('recruiter.jobs.create')"
                                :active="request()->routeIs('recruiter.jobs.create')">
                                {{ __('Post a Job') }}
                            </x-nav-link>
                        @elseif(Auth::user()->isCandidate())
                            <x-nav-link :href="route('candidate.applications.index')"
                                :active="request()->routeIs('candidate.applications.index')">
                                {{ __('My Applications') }}
                            </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}"
        ...
        <div class="pt-2 pb-3 space-y-1">
            @auth
                ...
                @elseif(Auth::user()->isRecruiter())
                    <x-responsive-nav-link :href="route('recruiter.jobs.index')"
                        :active="request()->routeIs('recruiter.jobs.*')">
                        {{ __('My Jobs') }}
                    </x-responsive-nav-link>
                    <x-responsive-nav-link :href="route('recruiter.jobs.create')"
                        :active="request()->routeIs('recruiter.jobs.create')">
                        {{ __('Post a Job') }}
                    </x-responsive-nav-link>
                @elseif(Auth::user()->isCandidate())
                    <x-responsive-nav-link :href="route('candidate.applications.index')"
                        :active="request()->routeIs('candidate.applications.index')">
                        {{ __('My Applications') }}
                    </x-responsive-nav-link>
